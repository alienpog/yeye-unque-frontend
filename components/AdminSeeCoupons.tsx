...
import AdminSeeCoupon from "./AdminSeeCoupon";
interface Props{
    id : number
    coupon_name? : string
    owner_name? : string
...
function AdminSeeCoupons() {
    ...
    useEffect(() =>{
        async function fetchquery(){
            const res = await fetch(`${BACKEND_URL}adminseecoupons/`)
    ...
    async function seachquery(e:FormEvent<HTMLFormElement>){
     e.preventDefault();
     const res = await fetch(`${BACKEND_URL}adminsearchcoupons/${post}/`)
     const data = await res.json();
     setData(data);
    }
  ...
  return (
    <div>
        ...
        border-none  bg-transparent" placeholder="Seach Coupon..." />
        </div>
        ...
        {data.map((item) => <AdminSeeCoupon item ={item} key={item.id} />)}
        </div>
        </div>
  )
}

export default AdminSeeCoupons