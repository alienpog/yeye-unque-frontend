...
function Comments({ id, details }: Props) {
  ...
  const formattedNumber = formatNumber(commentData.length? commentData.length : 0);
  ...
  const postComment = async (e : FormEvent<HTMLFormElement>) => {
        ...
        fetch(`${BACKEND_URL}postcomment/${id}`,
        {
          method: 'POST',
          headers: {
  ...
  return (
    ...
    <div className={`${!commentData.length && 'h-0 overflow-hidden'} ${details && "mt-4"} ${!details && "relative"}`}>
     {!details && <div className='absolute bottom-0 left-0 pb-3 pl-3 h-6 bg-gradient-to-t from-[#F2F2F2] z-10 w-full '/>}
      ...
      <div className={`${details? "h-[100px] sm:h-[150px]":"h-[60px]"} overflow-y-scroll scrollbar-thumb-rounded-md overflow-x-auto scrollbar-thumb-[#E7D6CE] scrollbar-thin mb-2 px-1 py-3`}>
    ...
    {details && Number(getId) == id && (<form className="shadow-lg flex items-center justify-center space-x-1 h-9 md:h-10 mt-2 rounded-lg mb-4" onSubmit={postComment} >
