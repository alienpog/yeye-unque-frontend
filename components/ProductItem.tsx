...
function ProductItem({id, name, image, price, modelimages, old_price, truecon }: props){
  const router = useRouter();
  
  return (
    <div className={`mb-2 ${truecon && "w-[320px]"}`}>
        <div className='relative h-[300px] overflow-hidden mb-2 rounded-3xl'>
        <SeeMoreproduct id={id} images={modelimages}/>
        ...
        <p onClick= {() => router.push(`/productdetails/${id}`)} className='text-xs text-[#333333] font-bold text-center py-1 sm:py-2 bg-[#E7D6CE] rounded-full shadow-lg hover:shadow-none transition ease-in duration-300 cursor-pointer'>
        Check Design
        </p>
      </div>
